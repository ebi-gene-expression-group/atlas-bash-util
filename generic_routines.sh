...
get_organism_from_condensed_sdrf(){
    echo "DEBUG: Processing condensed SDRF file: $1" >&2
    result=$(cut -f 4,5,6 $1 | grep characteristic$'\t'organism$'\t' | head -n1 | cut -f 3 | to_ensembl_species_lowercase)
    echo "DEBUG: Extracted organism: $result" >&2
    echo "$result"
}
